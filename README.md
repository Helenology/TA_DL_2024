...
- [TA\_Deep\_Learning\_Manual](#ta_deep_learning_manual)
  ...
  - [3. 学期末](#3-学期末)
    ...
    - [3.2 报告](#32-报告)
  - [4. 期末打分](#4-期末打分)
    - [4.1 平时成绩（80%） `@jx`](#41-平时成绩80-jx)
    - [4.2 大作业（20%） `@bc`](#42-大作业20-bc)
      - [4.2.1 PPT](#421-ppt)
      - [4.2.2 报告](#422-报告)
    - [4.3 缺勤扣分 `@bc`](#43-缺勤扣分-bc)
    - [4.4 总成绩提交 `@qh`](#44-总成绩提交-qh)
  - [5. 教学经验记录](#5-教学经验记录)
...
- [ ] 课前准备 🅰️🅱️🅾️
 ...
 - **周日早上**问老王今天想喝什么饮料，从老楼咖啡厅**签单**该饮料，拿到教学楼，他一般喜欢喝柚子茶/美式（2023年新情报：我们随机拿一杯柚子茶/美式，不用请示老王）
 - 教学工具及可能的bug：
   - 老王钦点的饮料（从老楼一楼咖啡厅签单后免费领取）
   - 两瓶农夫山泉（从老楼一楼保安处签字后免费领取）
   - 纸质版课件
   - 拷贝PPT到教室电脑上，*可能会存在自动播放的bug，检查视频是否能够播放*，**将PPT转成PDF发到群里**
   - 翻页笔，*可能没插电池*
...
| 第6次课 11月3日 | `qh` | `bc` | `jx` | `bc` `qh` `jx` |
| 第7次课 11月10日 | `bc` | `jx` | `qh` | `qh` `jx` `bc` |
| 第8次课 11月17日 | `jx` | `bc` | `qh` | `jx` `bc` `qh` |


## 3. 学期末 

- 还剩两个分数：1. PPT成绩；2. 报告成绩，两个加权后作为总的大作业成绩

### 3.1 PPT

【汇报前】 `@jx`
1. 打印 3 份纸质版打分表
2. 拷贝 PPT 至教室电脑上，并禁止修改
3. 课前 10 min 现场抽签，本次采取朴素的抓阄方法

【汇报前+中】`@qh`
1. 给专家团准备茶歇
2. 展示时间举牌 (每组 20 min + 5min QA)，还有 5 min 和 2 min 时举牌，结束时直接打断

【汇报后】`@bc`
1. 负责【大作业成绩】的整理和计算（加权【PPT成绩】+【报告成绩】），详见 4.2 节
...
- **PPT成绩表**:
  ...
  |  组名   | 组员学号  | 组员姓名  | 专家i给分（一个专家一列）  | PPT平均分  |
  |  ----  | ----  | ----  | ----  | ----  |

#### 4.2.2 报告 

- 分数：助教评分后取平均 `qh` `jx` `bc`
- **报告成绩表**:
